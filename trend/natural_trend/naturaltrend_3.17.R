...
#select your samples
selected_groups <- c("0yr Dry", "0yr Wet")
...
results <- data.frame(
  ...
  C0 = numeric(), Cx = numeric(), Cf = numeric(), Tau1 = numeric(), Tau2 = numeric(), Sigma1 = numeric(), Sigma2 = numeric(),
  ...
  for (group in selected_groups) {
    ...
    if (is_monotonic_decreasing) {
      fit <- tryCatch(nlsLM(
        ...
        data = group_data,
        start = list(tau = max(group_data$ElapsedTime) / 2),#set the innitial tau
      ...
      if (!is.null(fit) && (abs(correlation) < 0.7)) {
        ...
        if (correlation >= 0.7) {
          ...
          message(paste("Data neither monotonic decreasing nor increasing for", group, "- no suitable fallback model. Correlation:", correlation))
        }
      }
    }
